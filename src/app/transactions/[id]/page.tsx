                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-gray-900">{formatCurrencyWithRs(transaction.basePrice)}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-xl font-semibold text-gray-900">{formatCurrencyWithRs(transaction.totalPrice)}</p>
                    </div>
                  </div>
            ...
            <div className="p-6">
              ...
              <div className="flex items-start space-x-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">Year: {transaction.vehicleId.year}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
            <div className="p-6">
              ...
              <div className="flex items-start space-x-4">
                ...
                <div>
                  <h4 className="text-xl font-semibold text-gray-900">{counterpartyName}</h4>
                  <p className="text-gray-600 capitalize">{transaction.counterpartyId.type}</p>
                  {transaction.counterpartyId.phone && transaction.counterpartyId.phone.length > 0 && (
                    <p className="text-sm text-gray-500">Phone: {transaction.counterpartyId.phone[0]}</p>
                  )}
                  {transaction.counterpartyId.email && (
                    <p className="text-sm text-gray-500">Email: {transaction.counterpartyId.email}</p>
                  )}
                </div>
              </div>
            </div>
                ...
                <div className="space-y-3">
                  {transaction.payments.map((payment: any, index: number) => (
                    <div key={index} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                      <div>
                        <p className="font-medium text-gray-900">{formatCurrencyWithRs(payment.amount)}</p>
